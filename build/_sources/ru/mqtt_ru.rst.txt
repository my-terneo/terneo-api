...
``Publish prefix``/``Client ID``/``floorTemp`` - показания датчика температуры пола, `телеметрия <telemetry_ru.html>`_ **"t.1"** в °C строкой "xx.x" (модели с датчиком пола)

``Publish prefix``/``Client ID``/``airTemp`` - показания датчика температуры воздуха, `телеметрия <telemetry_ru.html>`_ **"t.2"** в °C строкой "xx.x" (модели с датчиком воздуха)
